...
In-paper anchor: `paper/P_ADIC_HODGE_REALIZATIONS_PREPRINT.md` (bridge and final strict margin).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of admissible comparison isomorphisms and period-filtration realization data across a multi-lane p-adic Hodge realization super-architecture`.
...
2. the endpoint-transfer lock `hodge_tate_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_PHR` remains positive.
...
Payload: verify `hodge_tate_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_PHR` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `hodge_tate_transfer`
