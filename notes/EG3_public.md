...
In-paper anchor: `paper/P_ADIC_HODGE_REALIZATIONS_PREPRINT.md` (`PHR_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of admissible comparison isomorphisms and period-filtration realization data across a multi-lane p-adic Hodge realization super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`PHR_G3` closes when `kappa_compact` provides the compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_PHR`.
...
If near-failure extraction and no-Zeno spacing hold, then `PHR_G3` prevents hidden drift before rigidity is applied.
