...
Canonical Lane super-repo for a flagship umbrella problem in p-adic geometry, Galois representations, and period theory:
proving persistence of admissible comparison isomorphisms and period-filtration realization data across a multi-lane p-adic Hodge realization super-architecture.

## Main Manuscript

- [paper/P_ADIC_HODGE_REALIZATIONS_PREPRINT.md](paper/P_ADIC_HODGE_REALIZATIONS_PREPRINT.md)
...
1. Hodge-Tate, de Rham, crystalline, and semistable comparison packages,
2. filtered phi-module and period-ring compatibility packages,
3. local monodromy and Sen-operator transport packages,
4. integral lattice and coefficient-extension stabilization packages,
5. local-global stitching of the declared realization-filtration endpoint package.
...
- `paper/`
  - `P_ADIC_HODGE_REALIZATIONS_PREPRINT.md`
...
1. Super-lane theorem chain first: read `paper/P_ADIC_HODGE_REALIZATIONS_PREPRINT.md`.
...
Current PHR runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/phr_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/phr_closure_guard.py`.
...
- Manuscript target: [paper/P_ADIC_HODGE_REALIZATIONS_PREPRINT.md](paper/P_ADIC_HODGE_REALIZATIONS_PREPRINT.md)
